...
const ALLOWED_FIELDS: Array<keyof RuleFieldsCustomizationMap> = [
  'name',
  'description',
  'risk_score',
  'severity',
...
const initRuleFieldsCustomizationMap = (): RuleFieldsCustomizationMap => ({
  name: 0,
  description: 0,
  risk_score: 0,
  severity: 0,
...
export const getRuleCustomizationStatus = (
...
): RuleFieldsCustomizationMap => {
  const counts = initRuleFieldsCustomizationMap();
  ...
  ruleSources.forEach((ruleSource) => {
    ...
    ruleSource.customized_fields.forEach((field) => {
      const fieldName = field.fieldName as keyof RuleFieldsCustomizationMap;
