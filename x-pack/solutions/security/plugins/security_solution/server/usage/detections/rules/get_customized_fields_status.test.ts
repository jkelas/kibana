...
import { QueryRule } from '@kbn/security-solution-plugin/common/api/detection_engine/model/rule_schema/rule_schemas.gen';
...
describe('getCustomizedFieldsStatus', () => {
  ...
  it('returns empty breakdown for a non-customized rule', async () => {
    ...
    // // Patch calculateRuleDiff and batch logic
    // jest
    //   .spyOn(require('./get_customized_fields_status'), 'calculateFieldsDiffsInBatches')
    //   .mockResolvedValue([
    //     {
    //       ruleDiff: {
    //         fields: {
    //           my_field: { diff_outcome: 'BASE=A, CURRENT=B, TARGET=A' },
    //           other_field: { diff_outcome: 'BASE=A, CURRENT=A, TARGET=A' },
    //         },
    //       },
    //     },
    //   ]);
